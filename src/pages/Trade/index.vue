  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="userAddress, index in userAddressList" :key="userAddress.id">
        ...
        <p @click="changeIsDefault(userAddress, userAddressList)">
      ...
      <div class="detail">
        <h5>商品清单</h5>
        <ul class="list clearFix" v-for="detail,index in orderInfo.detailArrayList" :key="detail.skuId" v-if="orderInfo.detailArrayList">
          <li>
    ...
    <div class="sub clearFix">
      <a class="subBtn" to="/pay" @click.prevent="goPay">提交订单</a>
    </div>
  </div>
</template>
...
export default {
  ...
  mounted() {
    this.$store.dispatch('reqUserAddressActions')
    this.$store.dispatch('reqOrderInfoActions')
  },
  computed: {
    userAddressList() {
    ...
    sendAddress(){
      // 这个是返回数组
     /* var data = this.userAddressList.filter((item) => {
  ...
  methods: {
    ...
    /*     consignee:this.sendAddress.consignee,
        ...
        consignee:this.$store.state.register.UserInfo.name,
        consigneeTel:this.$store.state.register.UserInfo.phoneNum,
        deliveryAddress:"北京",
        paymentWay:'ONLINE',
