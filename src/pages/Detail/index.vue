          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="item in spuSaleAttrList" :key="item.id">
                ...
                <dd changepirce="0" :class="{ active: item2.isChecked == 1 }" v-for="item2 in item.spuSaleAttrValueList"
                  :key="item2.id" @click="changeColor(item.spuSaleAttrValueList, item2)">{{ item2.saleAttrValueName }}</dd>
              </dl>
              <!-- <dl>
            ...
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" @blur="changeSkuNum($event)" v-model="skuNum">
                <a href="javascript:" class="plus" @click="skuNum++">+</a>
                <a href="javascript:" class="mins" @click="skuNum>1?skuNum--:skuNum = 1">-</a>
              </div>
              <div class="add">
                <a  @click="goAddCartList(skuInfo)">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    changeColor(spuSaleAttrValueList, item) {
      // 修改产品售卖属性
    ...
    async goAddCartList(skuInfo){
      // 把用户选的购物车的信息，提前给服务器存储
     let skuId = this.$route.params.skuId
     try{
        await this.$store.dispatch('reqAddtoCardListActions',{skuId:skuId,skuNum:this.skuNum})
