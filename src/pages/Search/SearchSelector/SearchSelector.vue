  ...
  <div class="clearfix selector">
    <div class="type-wrap logo">
      ...
      <div class="value logos">
        <ul class="logo-list">
          <li v-for="item in listData.trademarkList " :key="item.tmId" @click="searchTrademarkDetail(item)">{{ item.tmName
          }}</li>
          <!-- <li>TCL</li>
                <li>长虹（CHANGHONG）</li>
                <li>飞利浦（PHILIPS）</li>
    ...
    <div class="type-wrap" v-for="attr in listData.attrsList" :key="attr.attrId">
      <div class="fl key">{{ attr.attrName }}</div>
      <div class="fl value">
        <ul class="type-list">
          <li v-for="attrValue, index in attr.attrValueList" :key="index" @click="setPropsList(attr, attrValue)">
            <a>{{ attrValue }}</a>
          </li>
          <!--  <li>
                  <a>电信2G</a>
...
export default {
  ...
  methods: {
    searchTrademarkDetail(trademark) {
      this.$emit('searchTradeMark', trademark)
    },
    setPropsList(attr, attrValue) {
      this.$emit('getPropsList', attr, attrValue)
    }
  },
