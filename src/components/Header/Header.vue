<template>
    <!-- 头部 -->
    <header class="header">
        <!-- 头部的第一行 -->
        <div class="top">
            <div class="container">
                <div class="loginList">
                    <p>百年奥莱欢迎您！</p>
                    <p>
                        <span v-if="name == ''">请</span>
                        <router-link to="/login" v-if="name == ''">登录</router-link>
                        <router-link to='/register' class="register" v-if="name == ''">免费注册</router-link>
                        <a v-if="name">{{ name }}</a>
                        <a v-if="name"  class="register" @click="Logout">退出登录</a>
                    </p>
                </div>
                <div class="typeList">
                    <router-link to="/center/centerorder" >我的订单</router-link>
                    <router-link to="/shopcart">我的购物车</router-link>
                    <a href="###">我的百年奥莱</a>
                    <a href="###">百年奥莱会员</a>
                    <a href="###">企业采购</a>
                    <a href="###">关注百年奥莱</a>
                    <a href="###">合作招商</a>
                    <a href="###">商家后台</a>
                </div>
            </div>
        </div>
        <!--头部第二行 搜索区域-->
        <div class="bottom">
            <h1 class="logoArea">
                <router-link to="/home" class="logo">
                    <img src="./images/logo.jpg" alt="">
                </router-link>
            </h1>
            <div class="searchArea">
                <form action="###" class="searchForm" @submit.prevent>
                    <input type="text" id="autocomplete" class="input-error input-xxlarge" v-model="keyword"
                        v-on:keyup.enter="goSearch" />
                    <button class="sui-btn btn-xlarge btn-danger" @click='goSearch' type="button">搜索</button>
                </form>
            </div>
        </div>
    </header>
</template>

<script>

export default {
    data() {
        return {
            keyword: '',
        }
    },
    methods: {
        goSearch($event) {
            let location = {
                name: 'search',
                params: { keyword: this.keyword || undefined }
            }
            if (this.$route.query) {
                location.query = this.$route.query
            }
            this.$router.push(location)
        },
        async Logout(){
           await this.$store.dispatch('reqLogoutActions')
           this.$router.push({path:'/home'})
        }
    },
    mounted() {
        // 清空搜索框
        this.$bus.$on('clearKeyword', () => {
            this.keyword = ''
        })
    },
    computed:{
        name(){
            return this.$store.state.register.name
        }
    }
    
}
</script>

<style lang="less">
.header {
    &>.top {
        background-color: #eaeaea;
        height: 30px;
        line-height: 30px;

        .container {
            width: 1200px;
            margin: 0 auto;
            overflow: hidden;

            .loginList {
                float: left;

                p {
                    float: left;
                    margin-right: 10px;

                    .register {
                        border-left: 1px solid #b3aeae;
                        padding: 0 5px;
                        margin-left: 5px;
                    }
                }
            }

            .typeList {
                float: right;

                a {
                    padding: 0 10px;

                    &+a {
                        border-left: 1px solid #b3aeae;
                    }
                }

            }

        }
    }

    &>.bottom {
        width: 1200px;
        margin: 0 auto;
        overflow: hidden;

        .logoArea {
            float: left;

            .logo {
                padding: 20px;
                img {
                    width: 170px;
                    margin: 25px 0;
                    margin-right: 55px;
                }
            }
        }

        .searchArea {
            float: right;
            margin-top: 35px;

            .searchForm {
                overflow: hidden;

                input {
                    box-sizing: border-box;
                    width: 490px;
                    height: 32px;
                    padding: 0px 4px;
                    border: 2px solid #ea4a36;
                    float: left;

                    &:focus {
                        outline: none;
                    }
                }

                button {
                    height: 32px;
                    width: 68px;
                    background-color: #ea4a36;
                    border: none;
                    color: #fff;
                    float: left;
                    cursor: pointer;

                    &:focus {
                        outline: none;
                    }
                }
            }
        }
    }
}
</style>